...
public class movePlayer : MonoBehaviour
{
    ...
    void attack()
    {
        if (Input.GetKeyDown(KeyCode.J))
        {
            ...
            if (!flip.flipX)
            {
                _attack.transform.localPosition = new Vector3(1, 0, 0);
                _attack.transform.localScale = new Vector3(1, 1, 1);
            }
            else if (flip.flipX)
            {
                _attack.transform.localPosition = new Vector3(-1, 0, 0);
    ...
    public void GetHit(float dame)
    {
        ...
        Vector3 pos_player = this.transform.position;
        pos_player.x += pos_x;
        pos_player.y += pos_y;
        GameObject d = Instantiate(Damage_PopUp, pos_player, Quaternion.identity);
